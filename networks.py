...
def big_conv_lstm_readout_net(feature_map_in_seqs, feature_map_size, drop_rate, gaze_prior=None):
    ...
    initial_c = layers.Conv2D(1, (1, 1), activation='tanh')(tf.reduce_mean(x, axis=1))
    initial_h = layers.Conv2D(1, (1, 1), activation='tanh')(tf.reduce_mean(x, axis=1))
    
    conv_lstm = layers.ConvLSTM2D(filters=1,
                                  kernel_size=(3,3),
                                  strides=(1,1),
                                  padding='same', 
                                  dropout=drop_rate, 
    ...
    x = conv_lstm(x, initial_state=[initial_c, initial_h])
